...
This file contains the actual analysis code for my parts of the project. It starts with `allSRAFinal.rds`, which is generated at the end of `02_clean_ancestry_terms.Rmd` but does the actual analyses of diversity across tissues and studies and all that goodness. 
...
The reason I went down this rabbit hole was that tehre were about 3000 samples that didn't have tissue info if we focused on the most complete categories: `Cell_type` and `Tissue`. I checked some manually and most of them seemed to have info in `source_name`, which I then rescued from the original SRA downloads... so let's look at whether we really need the info from the other columns too:
...
dim(allSRAFinal[is.na(allSRAFinal$Tissue) & is.na(allSRAFinal$Cell_type) & is.na(allSRAFinal$source_name) & is.na(allSRAFinal$tissue_type), ])

noInfo <- allSRAFinal[is.na(allSRAFinal$Tissue) & is.na(allSRAFinal$Cell_type) & is.na(allSRAFinal$source_name) & is.na(allSRAFinal$tissue_type), ]
...
* ERP001942: TSI, CEU, etc - either LCLs or nothing, but eitehr way we would not want to keep it? Project title is "RNA-sequencing of 465 lymphoblastoid cell lines from the 1000 Genomes" so set to LCL? Info recorded in 'strain' according to runselector
* ERP116722: Bunch of African inds with malaria, but no tissue info in any of the columns I kept; grabbed the raw data from run selector and it's in a column called `Organism_part`.
* ERP117085: Blood, according to project title, recorded in `Organism part`
* ERP122083: Some sort of cancer sample, maybe? Tissue provenance recorded in `Sampling_site` and also `Organism_part`, and it's unclear if these are healthy biopsies or cancer biopsies or both.
* ERP122103: Sampling site info is in `Organism_part`, but like above, pretty sure these are cancer biopsies? 
* ERP122256: Sampling site info is in `Organism_part`, but once again... cancer biopsies? Can't tell...
