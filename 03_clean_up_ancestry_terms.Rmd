---
title: "Cleaning up the ancestry terms"
...
allSRAClean <- readRDS("20250517_allSRAClean.rds")
```

### Cleaning up descriptors:

After a lot of discussion, it's worth having one more look at how the population descriptors were cleaned up and converted, so we write them all out into a huge table (in 2024, not updated since, but this is a subset) and then agree/disagree into what they get classified under.

```{r pop-descriptors-clean, include=F, eval=F}
...
saveRDS(allSRAFinal, file="20250517_allSRAAncestry.rds")
```
