...
old <- fread("SRA_metadata/20240613_run_selector_metadata.txt", na.strings = "", sep="\t")
new <- fread("SRA_metadata/20240613_run_selector_metadata_post2019.txt", na.strings = "")
...
# First we need to fix the NAs in all the ancestry-associated columns... but this function only operates on vectors, so...
allSRAFilt$ETHNICITY <- na_if(allSRAFilt$ETHNICITY, "NA")
allSRAFilt$RACE <- na_if(allSRAFilt$RACE, "NA")
allSRAFilt$Population <- na_if(allSRAFilt$Population, "NA")
allSRAFilt$reported_race<- na_if(allSRAFilt$reported_race, "NA")
allSRAFilt$ancestry <- na_if(allSRAFilt$ancestry, "NA")
allSRAFilt$race.ethnicity <- na_if(allSRAFilt$race.ethnicity, "NA")
allSRAFilt$primary_race <- na_if(allSRAFilt$primary_race, "NA")
allSRAFilt$DONOR_ETHNICITY <- na_if(allSRAFilt$DONOR_ETHNICITY, "NA")
