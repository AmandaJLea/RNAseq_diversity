...
Files from Amanda and Grace are in different formats than the SRA one, so they need to be processed so we can do some joint plotting etc. Note that the analyses in this file haven't changed since it was last run, but I've updated the file names of the output to reflect the new date of running. 
...
saveRDS(dbgapMelt, file="20250518_dbgap_final.rds")
```
...
saveRDS(litRevSimple, file="20250518_manual_final.rds")
```
