...
```{r plot-clean-geography, out.width="100%"}
# First we clean up the plotting labels:
allSRAFinal <- allSRAFinal %>% group_by(finalRace) %>%
               mutate(finalRace = gsub("or ", "or\n", finalRace)) %>% as.data.frame()

allSRAFinal <- allSRAFinal %>% group_by(finalGeography) %>%
               mutate(finalGeography = gsub("and ", "and\n", finalGeography)) %>% as.data.frame()
...
```{r geography-by-study, include=T, eval=T, out.width="100%"}
...
```{r race-by-study, include=T, eval=T, out.width="100%"}
