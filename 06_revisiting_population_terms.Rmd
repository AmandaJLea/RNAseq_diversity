...
3. **Manual review** Leave things as they were with the original reclassification that we used, which does make some assumptions - caucasian is always racial, and white gets moved around a bit. I don't like it because it is arbitrary.

Let's go in order:

### Option 1: The original category label trumps all:

This method is clearly incorrect, but worth exploring anyhow, to see how bad the classification is if we are strict (worth reporting in a supplement). All of the code used here and below is borrowed from `02_clean_ancestry_terms.Rmd` but without most of the intermediate qc because this is meant to be brutal and automatic.
...
table(!is.na(allSRAFinal$strictestGeography), !is.na(allSRAFinal$strictestRace)) # Done.
```
