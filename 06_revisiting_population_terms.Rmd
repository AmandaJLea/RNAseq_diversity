...
allSRAFinal <- readRDS("20250517_allSRAAncestryTissuesDiseaseCenters.rds")
```
...
## 2: Resolving ambiguity in the population descriptors:
...
1. **Be very strict**: if the column says race, then it's a race, if the column says ethnicity, then I don't care what your term is, it's a geographic origin. This will over-correct, but should perhaps have been our starting position, to see how things improve with other choices. It should also resolve the problem of studies being split across descriptor categories. *This is the approach used in the paper, and coded in all downstream analyses as `strictestRace` and `strictestGeography`. All terms are available in Table S6 in the paper*
2. **Alternative 1: Manual curation** Leave things as they were with the original reclassification that we used, which does make some assumptions - caucasian is always racial, and white gets moved around a bit. This was our initial approach and was explored in more depth in `03_clean_up_ancestry_terms`, see that script for a more detailed breakdown. *Our full recoding under this schema is presented in Table S6 in the paper under the "alternative" heading, and coded in all downstream analyses as `finalRace` and `finalGeography`.* (Yes I know the name is misleading)
3. **Alternative 2: Be somewhat strict**: We had previously decided that certain terms (Caucasian, white, African, black) were racial no matter the column label, which is perhaps a problem, so here I have left some terms that are not ambiguous but are clearly in the wrong column unchanged (eg African American or Native Hawaiian is racial, but black is not because it is used outside the USA without reference to the USA context). This will bungle some stuff and I suspect the moving of caucasian and white and black terms will make things way worse when we consider descriptor use within a study. *This is coded below as downstream analyses as `strictRace` and `strictGeography` but not presented further in the final manuscript.*

### Strictest approach: The category label trumps all.

This method is worth exploring, to see how things look if we are strict (worth reporting in a supplement). All of the code used here and below is borrowed from `03_clean_up_ancestry_terms` but without most of the intermediate qc because this is meant to be brutal and automatic.

This is coded as `IGR.term.strictest` in the schema, and as strictestRace/Geography in the file.

```{r strictest-terms}
popDescriptors <- read.csv("20250518_descriptor_terms_new_schema.csv")
...
geographyTerms <- allSRAFinal %>% count(strictestGeography, mergedGeography)
raceTerms <- allSRAFinal %>% count(strictestRace, mergedRace) 
...
### Alternative 1: Intensive manual curation.

Not repeating this, since it's what we did to begin with in `03_clean_up_ancestry_terms`. Captured in the columns `finalRace` and `finalGeography` in the object here.


### Alternative 2: The original category label trumps most things.
...
This is coded as `IGR.term.strict` in temp files.
...
Added 20260301: Making reviewer 2 a table of how race and geography assignments vary between `final` and `strictest`.

```{r compare-assignments}

raceGeogAlluvial <- allSRAFinal %>%
  select(strictestRace, strictRace, finalRace,
         strictestGeography, strictGeography, finalGeography) %>%
  mutate(
    strictest = paste0("Race: ", strictestRace, " Geography: ", strictestGeography),
    strict    = paste0("Race: ", strictRace, " Geography: ", strictGeography),
    final     = paste0("Race: ", finalRace, " Geography: ", finalGeography)
  ) %>%
  mutate(
    strictest = gsub("Race: NA ", "", strictest),
    strictest = gsub("Geography: NA", "", strictest),
    strict = gsub("Race: NA ", "", strict),
    strict = gsub("Geography: NA", "", strict),
    final = gsub("Race: NA ", "", final),
    final = gsub("Geography: NA", "", final),
  ) %>%
  count(strictest, strict, final)

ggplot(data = raceGeogAlluvial, aes(axis1 = strictest, axis3 = strict, axis2 = final, y = n)) +
  scale_x_discrete(limits = c("Strictest", "Alternative 1", "Alternative 2"), expand = c(.3, .1)) +
  geom_alluvium(aes(fill = strictest)) +
...
ggsave("descriptor_alluvial_for_reviewers.pdf", width = 6, height = 5)
ggsave("descriptor_alluvial_for_reviewers.png", width = 6, height = 5)
...
saveRDS(allSRAFinal, file="20250517_allSRAFinal.rds")
```
