...
Grab all the SRA IDs that were deposited by GEO and let's figure out where we go from there... (Note eval = F for ease of rendering)

```{r depositing-site, include=T, eval=F}
...
write.table(getSubmitterdf, file = "20250629_final_Site_SRA_results.tsv", sep="\t", quote=F, eol="\n")
```
...
```{r depositing-site-2, include=T}
getSubmitterdf <- read.table("20250629_final_Site_SRA_results.tsv", header=T, sep="\t")
...
table(is.na(allSRAFinal$finalSite))

# This is very nice but there's a problem... which is that some studies have multiple submitters. So we do this by institution name instead.  

for (i in 1:nrow(countryList)){
 tryCatch({
  allSRAFinal[allSRAFinal$finalSite %in% countryList$finalSite[i],]$finalCountry <- countryList$Country[i]}, error = function(e) {
    message(sprintf("Error at i = %d: %s", i, e$message))
  })    
}
