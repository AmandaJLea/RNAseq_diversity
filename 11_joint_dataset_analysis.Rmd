...
allSRAClean %>% drop_na(finalCountry) %>% group_by(finalCountry) %>% summarise(samples = n()) %>% mutate(prop = samples/sum(samples)) %>% as.data.frame()

allSRAClean %>% drop_na(finalCountry) %>% group_by(worldBank) %>% summarise(samples = n()) %>% mutate(prop = samples/sum(samples)) %>% as.data.frame()

# By study:
allSRAClean %>% drop_na(finalCountry) %>% group_by(finalCountry) %>% summarise(studies = n_distinct(SRA.Study)) %>% mutate(prop = studies/sum(studies)) %>% as.data.frame()

allSRAClean %>% drop_na(finalCountry) %>% group_by(worldBank) %>% summarise(studies = n_distinct(SRA.Study)) %>% mutate(prop = studies/sum(studies)) %>% as.data.frame()

# Let's quickly do a test for how many studies and samples come from HIC relative to all studies, given in the text as 74:
hicStudyTest <- allSRAClean %>% drop_na(finalCountry) %>% group_by(worldBank) %>% summarise(studies = n_distinct(SRA.Study)) %>% mutate(prop = studies/sum(studies)) %>% as.data.frame()
...
Since we're here, we'll add a couple of analyses looking at the USA submitted samples in general vs non-USA submitted samples... but the question is, which? And do we use Jaro-Winklar 0 or 0.05?
...
cleanCountries <- allSRAClean %>% count(SRA.Study, finalCountry)

compareCountries <- left_join(finalCountries, cleanCountries, join_by("SRA.Study" == "SRA.Study"))

# And now we tabulate:
table(is.na(compareCountries$finalCountry.y))
compareCountries %>% drop_na(finalCountry.y) %>% 
  mutate(finalCountry.y = gsub("United States", "USA", finalCountry.y)) %>% 
  distinct(SRA.Study, finalCountry.x, .keep_all = T) %>%
  count(finalCountry.x == finalCountry.y)
