...
allSRAClean <- readRDS("20250605_allSRAFilt_Razi_left_country.rds") # All samples with inferred country info
...
New version of figure 1 without the manual data and with smaller plots because there's only two bars now: 

# CURRENTLY NOT EVALUATED BECAUSE IT NEEDS BETTER INTEGRATION IN THE CODE.

```{r plot-clean-country-2-with-kasper, eval=F}
...
dbByRegion <- allCountry %>% group_by(worldRegion, db) %>% summarise(value = sum(value), db = db) %>% distinct() %>% as.data.frame %>% 
  ggplot(., aes(x = db, y=value, fill=worldRegion)) +
  geom_bar(stat = "identity") +
  ggtitle("Sample depositor location (WB Economic Region)") +
  xlab("Database") +
  ylab("Number of samples") +
  scale_fill_wber() +
  coord_flip() +
...
dbByEconomy <- allCountry %>% group_by(worldBank, db) %>% summarise(value = sum(value), db = db) %>% distinct() %>% as.data.frame %>% 
  ggplot(., aes(x = db, y=value, fill=worldBank)) +
  geom_bar(stat = "identity") +
  ggtitle("Sample depositor location (WB Income Group)") +
  xlab("Database") +
  ylab("Number of samples") +
  scale_fill_viridis_d(option = "viridis") +
  coord_flip() +
...
dbByRegion + dbByEconomy + plot_spacer() + 
  plot_layout(design = mixed3Design) +
  plot_annotation(tag_levels = 'A')

ggsave("no_manual_sample_properties_map_pre.pdf", width = 7, height = 3.5)
ggsave("no_manual_sample_properties_map_pre.png", width = 7, height = 3.5)
```
...
```{r sra-map}
toplot<-aggregate(BioSample~finalCountry,data=allSRACountry,FUN=length)
...
geoPlot + dbgapPlot + racePlot +
    plot_layout(design = threeDesign, tag_level = 'new') +
    plot_annotation(tag_levels = 'A') 

# Because this figure has been cleaned up already...
ggsave("no_manual_dbs_descriptors.pdf", width = 7, height = 4.5)
ggsave("no_manual_dbs_descriptors.png", width = 7, height = 4.5)

```

Adding the PCA plot from Kasper Hansen's paper (Razi et al, 2024, bioRxiv) NEEDS TO BE FIXED AND UPDATED - MOVE FROM PREVIOUS SCRIPT.

```{r fig2-pca-from-genotype, eval=F}
genoCalls <- read.csv("from_Kasper_suppTable3.csv")
table(genoCalls$pred_pop)

# Need to align our colour mappings with theirs... or come up with a new colour scheme???

pcaGenotype <- ggplot(genoCalls, aes(x = pc1, y = pc2, color = pred_pop)) +
  geom_scattermore()
  ggtitle("Principal components from inferred genotype") +
  xlab("PC1") +
  ylab("PC2") +
  scale_fill_dbGap() +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
