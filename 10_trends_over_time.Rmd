...
Prep for panel A - dbGap samples over time
```{r}
...
panelDOuter <- ggplot(tmp5, aes(x=Var1, y=cumulative, color=variable)) +
  geom_line(linewidth=1.2)+
  geom_point(size=3)+
  labs(x = "Year", y = "Cumulative Samples") +
  # theme_bw(13) +
...
# Assemble the insets
panelBJoint <- panelBOuter + inset_element(panelBInner, left=0.01, bottom=0.35, top=0.98, right=0.5)
panelCJoint <- panelCOuter + inset_element(panelCInner, left=0.01, bottom=0.35, top=0.98, right=0.5)
panelDJoint <- panelDOuter + inset_element(panelDInner, left=0.01, bottom=0.35, top=0.98, right=0.5)
