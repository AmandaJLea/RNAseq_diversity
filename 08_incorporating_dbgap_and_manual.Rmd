...
litRevSimple[is.na(litRevSimple$finalGeo) & is.na(litRevSimple$finalRace),] # Gotta fix all of the hispanic samples, and decide what to do with the Unknowns - I prefer not to plot those? 

litRevSimple <- litRevSimple %>%
  mutate(finalRace = ifelse(InterpretT == "hispanic", "hispanic", finalRace)) %>%
  mutate(finalRace = ifelse(is.na(finalRace) & grepl("SRP079684|SRP136469|SRP187431", recount3) , "Unknown", finalRace)) %>% 
  mutate(finalGeo = ifelse(is.na(finalGeo) & grepl("SRP144647", recount3), "Unknown", finalGeo))

litRevSimple[is.na(litRevSimple$finalGeo) & is.na(litRevSimple$finalRace),] # Gotta fix all of the hispanic samples, to 
...
litRevSimple %>%  
  summarise(totalInds = sum(individs)) # Correct
...
litRevSimple %>% filter(!is.na(finalGeo)) %>% 
  summarise(totalInds = sum(individs)) # Correct, expect 2143
...
litRevSimple %>% filter(!is.na(finalRace)) %>% 
  summarise(totalInds = sum(individs)) # Correct, expect 1264
