...
litRevSimple <- read.csv("20240812_LitReviewinfo_NoNA_simple.csv") # Actual data
...
litRevSimple <- litRevSimple %>% 
  mutate(tempRace = ifelse(ReportedS == "race", ReportedT, NA)) %>% 
  mutate(tempGeo = ifelse(ReportedS == "ancestry", ReportedT, NA))         

litRevRace <- litRevAncestries[litRevAncestries$ReportedS %in% "race",] 
litRevGeo <- litRevAncestries[litRevAncestries$ReportedS %in% "ancestry",]
...
litRevSimple[is.na(litRevSimple$finalGeo) & is.na(litRevSimple$finalRace),] # Of these three, only one is plottable, so we fix that one manually:

litRevSimple[litRevSimple$recount3 %in% "SRP114956",]$finalRace <- "Black or African American"
...
saveRDS(litRevSimple, file="20240812_manual_final.rds")
```
