...
```{r}
allSRAFinal<-readRDS('20250518_allSRAFinal_for_plotting.rds')
...
panelCOuter <- ggplot(tmp4, aes(x=as.numeric(Var1)+2011, y=cumulative, color=group)) +
  geom_line(linewidth=1.2) +
  geom_point(size = 3) +
  ggtitle("SRA (ancestral/geographic origin)") +
  labs(x = "Year", y = "Cumulative Samples") +
...
panelDOuter <- tmp4 %>% filter(tmp4$Var1 > 2011 & tmp4$Var1 < 2023) %>% 
...
ggsave("trends_over_time_all_euro.pdf", width = 7, height = 7)
ggsave("trends_over_time_all_euro.png", width = 7, height = 7)

((panelE / panelCJoint) | (panelDJoint / panelBJoint)) +
  plot_annotation(tag_levels = 'A') &
    theme(plot.tag = element_text(size = 10))

ggsave("trends_over_time_all_alt_euro.pdf", width = 7, height = 7)
ggsave("trends_over_time_all_alt_euro.png", width = 7, height = 7)

# A final three panel layout, with all legends on the bottom right. This one is annoying to make so I got help... 

## 1. Extract legends from each joint panel, forcing a single vertical column
leg1 <- cowplot::get_legend(panelDJoint & theme(legend.position = "right", legend.direction   = "vertical", legend.title.align = 1, legend.background = element_blank(), legend.justification = c(0, 0)))

leg2 <- cowplot::get_legend(panelCJoint & theme(legend.position = "right", legend.direction   = "vertical", legend.title.align = 1, legend.background = element_blank(), legend.justification = c(0, 0)))

leg3 <- cowplot::get_legend(panelBJoint & theme(legend.position = "right", legend.direction   = "vertical", legend.title.align = 1, legend.background = element_blank(), legend.justification = c(0, 0)))

## 2. Remove legends from the main panels
panelC_noleg <- panelCJoint & theme(legend.position = "none")
panelD_noleg <- panelDJoint & theme(legend.position = "none")
panelB_noleg <- panelBJoint & theme(legend.position = "none")

## 3. Stack the three legends into a single column and wrap as a panel
legend_col   <- cowplot::plot_grid(leg1, leg2, leg3, ncol = 3, align = "v")
legend_panel <- patchwork::wrap_elements(legend_col)

## 4. Final 2×2 layout with desired panel order and tags
(panelD_noleg + panelC_noleg) / (panelB_noleg + legend_panel) +
  plot_annotation(tag_levels = "A") &
  theme(
...
ggsave("trends_over_time_no_gwas_euro.pdf", width = 7, height = 7)
ggsave("trends_over_time_no_gwas_euro.png", width = 7, height = 7)
