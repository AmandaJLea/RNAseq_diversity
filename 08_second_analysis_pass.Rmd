...
We will finally analyse this data properly. This script makes plots for figures 3 and 4 (possibly now only a single figure), and most of the statistical analyses that underpin them, as well as most of the numbers describing the SRA data in the text.
...
```{r plotting-prep-1}
# Read in, and tabulate sample origin:
allSRAFinal <- readRDS("20250517_allSRAFinalRazi.rds")
sourceRepo <- allSRAFinal %>% mutate(
  repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
sourceRepo

sum(sourceRepo$n) # This is still not the right subset...

# The original numbers were:
sourceRaw <- data.frame(repo = c("DRA", "ENA", "SRA"), n = c(10384, 68190, 716497))
sourceRepo$n/sourceRaw$n

studyRaw <- data.frame(repo = c("DRA", "ENA", "SRA"), n = c(195, 1230, 19784))
...
# One final tally
sourceFinal <- allSRAFinal %>% mutate(
  repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
sourceFinal

sum(sourceFinal$n) # At last!

studyFinal <- allSRAFinal %>% distinct(SRA.Study, .keep_all=T) %>% 
  mutate(
    repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
studyFinal

sourceFinal$n/sourceRaw$n
studyFinal$n/studyRaw$n
```
...
```{r plot-survey-level-2, out.width="100%", echo=T}
...
# How many studies in each category?
sampleGeography %>% distinct(SRA.Study) %>% nrow()
sampleRace %>% distinct(SRA.Study) %>% nrow()
...
First we separate studies on the basis of whether they contain racial or geographic descriptors. All instances of both are studies that contain both geographic and Hispanic samples. 
...
length(intersect(geoSRA, raceSRA)) # So that's 11... and we don't love that. But last time there were 13 and we didn't do much about them...
...
allSRAFinal %>% filter(grepl(paste(intersect(geoSRA, raceSRA), collapse="|"), SRA.Study)) %>% count(strictestGeography, strictestRace)

# I want to see it by study, though:
allSRAFinal %>% filter(grepl(paste(intersect(geoSRA, raceSRA), collapse="|"), SRA.Study)) %>% group_by(SRA.Study) %>% count(strictestGeography, strictestRace) %>% as.data.frame()
...
# And now the numbers by country:
allSRAFinal %>% drop_na(finalCountry, strictestRace) %>% count(finalCountry, strictestRace)
...
# We want a quick t-test on whether these are the same number of samples (we know they are not)

euroStudies <- geographyClean %>% filter(strictestGeography == "Europe") %>% count(SRA.Study)
oceaniaStudies <- geographyClean %>% filter(strictestGeography == "Oceania") %>% count(SRA.Study)
africaStudies <- geographyClean %>% filter(strictestGeography == "Sub-Saharan Africa") %>% count(SRA.Study)

t.test(euroStudies$n, oceaniaStudies$n) 
```
...
fisher.test(descriptorChi[,c(2,4)]) # Odds ratio interpretation varies depending on order, but otherwise all good.
...
And now the overrepresentation stats using the population data too:

```{r overrepresentation-in-submission-sra-final}
sraCountry <- allSRAFinal %>% drop_na(finalCountry) %>% 
  count(finalCountry, worldRegion, worldBank) %>% 
  melt() %>% 
  arrange(worldRegion) %>% 
  mutate(db = "SRA") %>% 
  mutate(finalCountry = ifelse(grepl("USA", finalCountry), "United States", finalCountry))

worldPop <- read.csv("RNA-seq-diversity_world-population.csv")
worldPop$name <- gsub("Korea, South", "South Korea", worldPop$name)

sraCountry2 <- sraCountry %>% 
  group_by(finalCountry, db, worldRegion) %>% 
  summarise(value = sum(value)) %>%
  as.data.frame()

sraCountry3 <- left_join(sraCountry2, worldPop, join_by( "finalCountry" == "name"))

# And now we get some fancy new columns for plotting
sraCountry3 <- sraCountry3 %>% 
...
allSRAFinal %>% count(is.na(finalCountry))

# How many institutions?
allSRAFinal %>% drop_na(finalCountry) %>% summarise(studies = n_distinct(SRA.Study), countries = n_distinct(finalCountry))

# Overrepresentation of submitting country relative to global population:
sraCountry3

# We wanna test the USA, so:
usaSamples <- allSRAFinal %>% filter(finalCountry == "United States") %>% nrow()
allSamples <- allSRAFinal %>% drop_na(finalCountry) %>% nrow()
usaProp <- sraCountry3[sraCountry3$finalCountry %in% "United States", ]$fractPop[1]
...
latamSamples <- sraCountry3 %>% filter(worldRegion == "Latin America &\nCaribbean") %>% 
...
allSRAFinal %>% drop_na(strictestGeography, finalOrgan) %>% count(finalOrgan, strictestGeography) %>% group_by(finalOrgan) %>% summarise(geoGroups = length(finalOrgan)) %>% arrange(desc(geoGroups)) %>% as.data.frame
  
allSRAFinal %>% drop_na(strictestRace, finalOrgan) %>% count(finalOrgan, strictestRace) %>% group_by(finalOrgan) %>% summarise(raceGroups = length(finalOrgan)) %>% arrange(desc(raceGroups)) %>% as.data.frame
...
allSRAFinal %>% drop_na(strictestGeography, finalOrgan) %>% count(finalOrgan, strictestGeography) %>% group_by(strictestGeography) %>% summarise(geoGroups = length(strictestGeography)) %>% arrange(desc(geoGroups)) %>% as.data.frame
  
allSRAFinal %>% drop_na(strictestRace, finalOrgan) %>% count(finalOrgan, strictestRace) %>% group_by(strictestRace) %>% summarise(raceGroups = length(strictestRace)) %>% arrange(desc(raceGroups)) %>% as.data.frame

allSRAFinal %>% drop_na(strictestRace, finalOrgan) %>% count(finalOrgan, strictestRace) %>% group_by(strictestRace) %>% as.data.frame

allSRAFinal %>% drop_na(strictestRace) %>% filter(!grepl("White", finalRace)) %>% filter(is.na(finalOrgan))
allSRAFinal %>% drop_na(strictestRace) %>% filter(grepl("White", finalRace)) %>% filter(is.na(finalOrgan))
...
allSRAFinal %>% drop_na(finalDisease) %>% count(finalDisease) %>% summarise(finalDisease = finalDisease, n = n, proportion = n/sum(n)) %>% as.data.frame
...
### A new figure 4 that combines figures 3 and 4

Here we are simply playing with plots, no new analyses being done at all, but we remove legends from the previous figure 4 panels, because they're not needed. Miraculously it all fits.

```{r combine-tissue-disease}
geoDiseaseSlim2 <- geoDiseaseSlim +
    theme(legend.position="none")

raceDiseaseSlim2 <- raceDiseaseSlim +
    theme(legend.position="none")

geoOrganSlim + raceOrganSlim + geoDiseaseSlim2 + raceDiseaseSlim2 +
  plot_layout(design=twox2DesignRowWise) +
  plot_annotation(tag_levels = 'A') 

ggsave("fig4_tissue_disease_combined_plots.pdf", width=6, height=6)
ggsave("fig4_tissue_disease_combined_plots.png", width=6, height=6)
...
```{r allSRAOut}
saveRDS(allSRAFinal, file="20250518_allSRAFinal_for_plotting.rds")
```
