...
allSRAFinal %>% count(finalOrgan, strictestGeography) %>% drop_na(strictestGeography, finalOrgan) %>% group_by(finalOrgan) %>% summarise(geoGroups = length(finalOrgan)) %>% arrange(desc(geoGroups)) %>% as.data.frame
  
allSRAFinal %>% count(finalOrgan, strictestRace) %>% drop_na(strictestRace, finalOrgan) %>% group_by(finalOrgan) %>% summarise(raceGroups = length(finalOrgan)) %>% arrange(desc(raceGroups)) %>% as.data.frame
...
allSRAFinal %>% count(finalOrgan, strictestGeography) %>% drop_na(strictestGeography, finalOrgan) %>% group_by(strictestGeography) %>% summarise(geoGroups = length(strictestGeography)) %>% arrange(desc(geoGroups)) %>% as.data.frame
  
allSRAFinal %>% count(finalOrgan, strictestRace) %>% drop_na(strictestRace, finalOrgan) %>% group_by(strictestRace) %>% summarise(raceGroups = length(strictestRace)) %>% arrange(desc(raceGroups)) %>% as.data.frame
...
Here we are simply playing with plots, no new analyses being done at all, but we remove legends from the previous figure 4 panels, because they're not needed. Miraculously it all fits, otherwise
