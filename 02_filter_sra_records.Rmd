...
nrow(allSRA)
sourceAll <- allSRA %>% distinct(BioSample, .keep_all =T) %>%
  mutate(
    repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
sourceAll

# And at the study level? Because that's how the decision gets made:
studyAll <- allSRA %>% distinct(SRA.Study, .keep_all =T) %>%
  mutate(
    repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
studyAll
...
nrow(allSRAFilt)
source10 <- allSRAFilt %>% distinct(BioSample, .keep_all =T) %>%
  mutate(
    repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
source10

study10 <- allSRAFilt %>% distinct(SRA.Study, .keep_all =T) %>%
  mutate(
    repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
study10
...
length(unique(allSRAFilt$BioSample))
nrow(allSRAFilt)
sourcePop <- allSRAFilt %>% distinct(BioSample, .keep_all =T) %>%
  mutate(
    repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
sourcePop

studyPop <- allSRAFilt %>% distinct(SRA.Study, .keep_all =T) %>%
  mutate(
    repo = ifelse(grepl("^SR", SRA.Study), "SRA", ifelse(grepl("^ER", SRA.Study), "ENA", "DRA"))) %>%
  count(repo)
studyPop

sourcePop$n/source10$n
sourcePop$n/sourceAll$n

studyPop$n/studyAll$n
